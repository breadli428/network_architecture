...
def list_generator(frame_id, frame_data, lecu_num, lecu_list):
...
'''
        ...
        for j in range(12):
            index_body_sub = index_body_sub.replace("Voltage %d data_done" % (j + 1), '%d' % (lecu_list[i]['Voltage'][j] / 32767 * 100))
            index_body_sub = index_body_sub.replace("Voltage %d data" % (j + 1), '%d mV' % lecu_list[i]['Voltage'][j])
        for k in range(6):
            index_body_sub = index_body_sub.replace("Temperature %d data_done" % (k + 1), '%d' % (lecu_list[i]['Temperature'][k] / 190 * 100))
            index_body_sub = index_body_sub.replace("Temperature %d data" % (k + 1), '%d &#8451' % lecu_list[i]['Temperature'][k])
